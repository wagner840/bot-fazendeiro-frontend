    ...
    return (
      <div className="min-h-screen bg-leather-950 flex items-center justify-center p-4">
        <motion.div
          ...
          className="max-w-md w-full bg-leather-900 border border-green-700/50 rounded-western p-6 sm:p-8 text-center"
        >
          ...
          <h2 className="font-display text-2xl sm:text-3xl text-green-500 mb-4">Assinatura Ativa!</h2>
          <p className="text-parchment-300 mb-6">
            O servidor <strong className="text-parchment-100">{currentGuildName}</strong> possui uma assinatura válida.
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-leather-950 flex items-center justify-center p-4">
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="max-w-md w-full bg-leather-900 border border-rust-700/50 rounded-western p-6 sm:p-8 text-center"
      >
        {/* Icon */}
        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-rust-500/20 flex items-center justify-center mx-auto mb-6">
          {isTrialCase
            ? <Clock className="w-8 h-8 sm:w-10 sm:h-10 text-rust-500" />
            : <AlertTriangle className="w-8 h-8 sm:w-10 sm:h-10 text-rust-500" />}
        </div>

        {/* Title */}
        <h2 className="font-display text-2xl sm:text-3xl text-rust-500 mb-4">
          {isTrialCase ? 'Período de Teste Expirado' : 'Assinatura Expirada'}
        </h2>

        {/* Description */}
        <p className="text-parchment-300 text-sm sm:text-base mb-6">
          {isTrialCase
        ...
        <div className="mb-8 bg-leather-800/50 p-4 rounded-western border border-leather-700/50">
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown size={18} className={`text-parchment-400 transition-transform flex-shrink-0 ml-2 ${showDropdown ? 'rotate-180' : ''}`} />
            </button>
